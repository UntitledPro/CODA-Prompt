...
class Trainer:
    def __init__(
        ...
        self.task_names: List[str] = [str(i + 1) for i in range(self.num_tasks)]
    ...
    def train(self, avg_metrics):
        ...
        for i in range(self.max_task):
            ...
            for mkey in ["acc"]:
                ...
                np.savetxt(
                    save_file, np.asarray(temp_table[mkey]), delimiter=",", fmt="%.2f"
                )
    ...
    def summarize_acc(self, acc_dict, acc_table, acc_table_pt):
        ...
        avg_acc_history: List[float] = [0.0] * self.max_task
        for i in range(self.max_task):
            train_name = self.task_names[i]
            cls_acc_sum = 0
            for j in range(i + 1):
    ...
    def evaluate(self, avg_metrics):
        ...
        for i in range(self.max_task):
            ...
            for j in range(i + 1):
                val_name = self.task_names[j]
                metric_table_local["acc"][val_name][
                    self.task_names[i]
                ] = self.task_eval(j, local=True)
