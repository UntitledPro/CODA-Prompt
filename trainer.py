...
class Trainer:
    def __init__(self, args: Namespace, seed: int,
        ...
        train_transform = dataloaders.utils.get_transform(
            dataset=args.dataset, phase='train', aug=args.train_aug, resize_imnet=resize_imnet)
        test_transform = dataloaders.utils.get_transform(
            dataset=args.dataset, phase='test', aug=args.train_aug, resize_imnet=resize_imnet)
    ...
    def train(self, avg_metrics):
        ...
        for i in range(self.max_task):
            ...
            for mkey in ['acc']:
                ...
                np.savetxt(save_file, np.asarray(
                    temp_table[mkey]), delimiter=",", fmt='%.2f')
    ...
    def summarize_acc(self, acc_dict, acc_table, acc_table_pt):
        ...
        for i in range(self.max_task):
            ...
            for j in range(i + 1):
                ...
                avg_acc_pt_local[j, i,
                                 self.seed] = acc_table_pt[val_name][train_name]
    ...
    def evaluate(self, avg_metrics):
        ...
        for i in range(self.max_task):
            ...
            for j in range(i + 1):
                val_name = self.task_names[j]
                metric_table['acc'][val_name][self.task_names[i]
                                              ] = self.task_eval(j)
            for j in range(i + 1):
                val_name = self.task_names[j]
                metric_table_local['acc'][val_name][self.task_names[i]] = self.task_eval(
                    j, local=True)
