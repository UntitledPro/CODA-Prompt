...
class Block(nn.Module):
    ...
    def forward(self, x, register_hook=False, prompt=None):
        x = x + self.drop_path(
            self.attn(
                self.norm1(x), register_hook=register_hook, prompt=prompt
            )
        )
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        attn_drop_rate=0.0,
        drop_path_rate=0.0,
        norm_layer: Union[nn.LayerNorm, None] = None,
        ckpt_layer=0,
    ):
        """
        Args:
            ...
            if prompt is not None:
                if train:
                    p_list, loss, x = prompt.forward(
                        q, i, x, train=True, task_id=task_id, targets=targets
                    )
                    prompt_loss += loss
                else:
                    p_list, _, x = prompt.forward(
                        q, i, x, train=False, task_id=task_id, targets=None
                    )
