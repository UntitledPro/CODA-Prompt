...
class DualPrompt(nn.Module):
    def __init__(self, emb_d, n_tasks, prompt_param, key_dim=768):
        ...
        for e in self.e_layers:
            ...
            p = tensor_prompt(
                self.e_pool_size, self.e_p_length, emb_d, ortho=True
            )
            # [pool_size, k_dim]
    ...
    def forward(
        self, x_querry, layer_idx, x_block, train=False, task_id=None, **kwargs
    ):
        # e prompts
        e_valid = False
...
class ResNetZoo(nn.Module):
    def __init__(
        self, num_classes=10, prompt_flag=False, prompt_param: List[float] = []
    ):
...
class TopDownZoo(nn.Module):
    ...
    def cls_hint_loss(self, cls_hint: Tensor):
        ...
        hint_labels = torch.arange(cls_hint_task.size(1)).to(
            cls_hint_task.device
        )
        ...
        # )
        hint_logits = torch.cat(
            (hint_logits[:, :task_s], hint_logits[:, task_s:]), dim=1
        )
...
def vit_pt_imnet(
    out_dim, block_division=None, prompt_flag="None", prompt_param=None
):
    return ResNetZoo(
