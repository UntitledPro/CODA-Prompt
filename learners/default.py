...
class NormalNN(nn.Module):
    ...
    def learn_batch(self, train_loader, train_dataset, model_save_dir, val_loader=None):
        ...
        if need_train:
            ...
            for epoch in range(self.config['schedule'][-1]):
                ...
                self.log(
                    ' * Loss {loss.avg:.3f} | Train Acc {acc.avg:.3f}'.format(loss=losses, acc=acc))
    ...
    def criterion(self, logits, targets, data_weights):
        loss_supervised = (self.criterion_fn(
            logits, targets.long()) * data_weights).mean()
    ...
    def validation(self, dataloader, model=None, task_in=None, task_metric='acc', verbal=True, task_global=False):
        ...
        for i, (input, target, task) in enumerate(dataloader):
            ...
            if task_in is None:
                ...
                acc = accumulate_acc(output, target, task,
                                     acc, topk=(self.top_k,))
            else:
                ...
                if len(target) > 1:
                    if task_global:
                        ...
                        acc = accumulate_acc(
                            output, target - task_in[0], task, acc, topk=(self.top_k,))
    ...
    def data_weighting(self, dataset, num_seen=None):
        self.dw_k = torch.tensor(
            np.ones(self.valid_out_dim + 1, dtype=np.float32))
        # cuda
        if self.cuda:
